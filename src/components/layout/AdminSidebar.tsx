...
export function AdminSidebar({ collapsed = false, onToggle }: AdminSidebarProps) {
  ...
  return (
    <aside className="h-full w-full bg-[#f8d7df] text-slate-700 flex flex-col pb-4 shadow-[4px_0_18px_rgba(190,24,93,0.08)] overflow-y-auto overflow-x-hidden border-r border-[#efc3cd]">
      {/* 헤더 */}
      <div className={`flex items-center h-14 shrink-0 ${collapsed ? 'justify-center px-2' : 'justify-between px-4'}`}>
        {!collapsed && (
          <Link href="/admin" className="block hover:opacity-80 transition-opacity min-w-0 flex-1 mr-2">
            ...
            <span className="text-rose-600 text-xs font-bold">관리자 뷰 (회계장부)</span>
          </Link>
        )}
        {onToggle && (
          <button
            ...
            className="shrink-0 w-8 h-8 flex items-center justify-center rounded-lg bg-white/75 hover:bg-white text-rose-500 hover:text-rose-700 border border-[#efc3cd] shadow-sm transition-all"
          >
      ...
      <div className="h-px bg-[#efc3cd] mx-3 mb-3 shrink-0" />
            ...
            <div key={item.name}>
              <div className="flex items-center">
                <Link
                  ...
                  } ${isActive ? 'bg-white text-rose-700 font-black shadow-sm ring-1 ring-[#efc3cd]' : 'hover:bg-white/70 hover:text-rose-700 text-slate-650'}`}
                >
                ...
                {hasSub && (
                  <button
                    ...
                    className="w-7 h-7 flex items-center justify-center rounded-lg hover:bg-white/70 text-rose-500 hover:text-rose-700 transition-all shrink-0 mr-1"
                  ...
                  {item.sub!.map(sub => (
                    <Link
                      ...
                      className={`text-xs px-3 py-2 rounded-lg transition-all ${
                        pathname === sub.href
                          ? 'bg-white text-rose-700 font-bold shadow-sm ring-1 ring-[#efc3cd]'
      ...
      <div className="px-2 mt-3 space-y-1 shrink-0">
        {!collapsed && user && (
          <div className="px-3 py-2 rounded-xl bg-white/65 border border-[#efc3cd] shadow-sm">
