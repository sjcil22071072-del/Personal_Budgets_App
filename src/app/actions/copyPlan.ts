...
async function assertStaff() {
  ...
  if (!user) return { ok: false, error: '?∏Ï¶ù ?ÑÏöî', supabase, user: null }
  ...
  if (!profile || (profile.role !== 'admin' && profile.role !== 'supporter')) {
    return { ok: false, error: 'Í∂åÌïú???ÜÏäµ?àÎã§.', supabase, user }
  }
...
export async function copyMonthlyPlans(
  ...
  if (!ok || !user) return { copied: 0, skipped: 0, error: error ?? 'Í∂åÌïú???ÜÏäµ?àÎã§.' }
  ...
  if (!fromPlans || fromPlans.length === 0) {
    return { copied: 0, skipped: 0, error: '?ÑÏõî??Î≥µÏÇ¨??Í≥ÑÌöç???ÜÏäµ?àÎã§.' }
  }
