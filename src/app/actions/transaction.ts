...
export async function createTransaction(formData: FormData) {
  ...
  const category = (formData.get('category') as string) || 'ê¸°í?'
  const memo = formData.get('memo') as string
  const status = (formData.get('status') as 'pending' | 'confirmed') || 'pending'
  const is_expense = formData.get('is_expense') !== 'false'
  const payment_method = (formData.get('payment_method') as string) || 'ì²´í¬ì¹´ë“œ'
...
export async function updateTransactionImages(
  ...
  // StorageÂ·DB ì¡°ì‘?€ ?œë¹„??ë¡??´ë¼?´ì–¸???¬ìš© (RLS ?°íšŒ)
  ...
  if (receiptFile && receiptFile.size > 0) {
    ...
    if (uploadError) {
      return { error: `?ìˆ˜ì¦??…ë¡œ???¤íŒ¨: ${uploadError.message}` }
    }
    // DB?ëŠ” ê³µê°œ URL ?•ì‹?¼ë¡œ ?€??(?˜ì¤‘??signed URL ?ì„± ??ê²½ë¡œ ì¶”ì¶œ???¬ìš©)
  ...
  if (activityFile && activityFile.size > 0) {
    ...
    if (uploadError) {
      return { error: `?œë™?¬ì§„ ?…ë¡œ???¤íŒ¨: ${uploadError.message}` }
    }
    // DB?ëŠ” ê³µê°œ URL ?•ì‹?¼ë¡œ ?€??(?˜ì¤‘??signed URL ?ì„± ??ê²½ë¡œ ì¶”ì¶œ???¬ìš©)
  ...
  if (error) return { error: `?€???¤íŒ¨: ${error.message}` }

  revalidatePath(`/supporter/transactions/${transactionId}`)
  revalidatePath('/supporter/transactions')

  // ë²„í‚·??private?´ë?ë¡??´ë¼?´ì–¸?¸ì—??signed URL ë°˜í™˜
...
/**
 * ?¹ì • ?¹ì‚¬?ì˜ ?¹ì • ??ê±°ë˜ ? ì§œ ê¸°ì?) ?”ë³„ ê³„íš ëª©ë¡??ë°˜í™˜.
 * ê±°ë˜ ?±ë¡/?¸ì§‘??ê³„íš ?œë¡­?¤ìš´?ì„œ ?¬ìš©.
 */
