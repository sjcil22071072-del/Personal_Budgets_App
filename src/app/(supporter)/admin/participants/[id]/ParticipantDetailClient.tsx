      ...
      <main className="flex-1 w-full max-w-2xl mx-auto p-4 sm:p-6 flex flex-col gap-6">
            ...
            {isEditing ? (
              ...
              <div className="grid grid-cols-2 gap-5 text-sm">
                <div>
                  ...
                  <p className="font-bold text-zinc-800 mt-0.5">{participant.budget_start_date || OPERATION_START_DATE} ~ {participant.budget_end_date || OPERATION_END_DATE}</p>
                </div>
                <div>
        ...
        <section className="flex flex-col gap-3">
          ...
          {fundingSources.map((fs: any) => {
            ...
            return (
              <div key={fs.id} className={`p-5 rounded-3xl border shadow-[0_4px_20px_rgba(0,0,0,0.015)] transition-all ${
                isEditingThis ? 'bg-white border-zinc-200' :
                isActive ? 'bg-white border-zinc-200/80' : 'bg-zinc-50/70 border-zinc-200/50 opacity-75'
              }`}>
                {isEditingThis ? (
                  <div className="flex flex-col gap-3">
                    ...
                    <div className="grid grid-cols-2 gap-3">
                      <div>
                        <label className="text-zinc-400 text-xs font-bold block mb-1">지원 시작일</label>
                        <input type="date" value={fsForm.startDate} onChange={e => setFsForm({ ...fsForm, startDate: e.target.value })}
                          className="w-full px-3 py-2 border border-zinc-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-zinc-300 text-sm font-semibold" />
                      </div>
                      <div>
                        <label className="text-zinc-400 text-xs font-bold block mb-1">지원 종료일</label>
                    ...
                    <div className="flex gap-2 mt-1">
                      ...
                      <button onClick={() => setEditingFsId(null)} className="flex-1 py-2 bg-zinc-100 text-zinc-650 text-xs font-bold rounded-xl hover:bg-zinc-200 transition-colors">취소</button>
                    </div>
                  </div>
                ) : (
                  <>
          ...
          {showAddFs && (
            <div className="p-5 rounded-3xl bg-zinc-50/50 border border-zinc-200 shadow-sm animate-fade-in-down">
              ...
              <div className="flex flex-col gap-3">
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    <label className="text-zinc-500 text-xs font-bold block mb-1">지원 시작일</label>
                    <input type="date" value={newFs.startDate} onChange={e => setNewFs({ ...newFs, startDate: e.target.value })}
                      className="w-full px-3 py-2 border border-zinc-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-zinc-300 text-sm font-semibold" />
                  </div>
                  <div>
                    <label className="text-zinc-500 text-xs font-bold block mb-1">지원 종료일</label>
                ...
                <div className="flex gap-2 mt-1">
                  ...
                  <button onClick={() => { setShowAddFs(false); setNewFs({ name: '', monthlyBudget: 0, yearlyBudget: 0, startDate: '', endDate: '' }) }} className="flex-1 py-2.5 bg-zinc-100 text-zinc-650 text-xs font-bold rounded-xl hover:bg-zinc-200 transition-colors">취소</button>
                </div>
              </div>
            </div>
          )}
                ...
                <Link key={tx.id} href={`/supporter/transactions/${tx.id}`} className="p-4 rounded-2xl bg-white border border-zinc-200/80 flex justify-between items-center hover:border-zinc-300 hover:bg-zinc-50/30 transition-all active:scale-[0.99] shadow-[0_4px_20px_rgba(0,0,0,0.01)]">
                  ...
                  <div className="text-right flex flex-col items-end gap-1">
                    ...
                    <span className={`text-[9px] font-bold px-2 py-0.5 rounded-full border ${
                      tx.status === 'confirmed' ? 'bg-green-50 text-green-600 border-green-200' :
