...
export default async function ParticipantDetailPage({ params }: PageProps) {
  ...
  const totalMonthlyBudget = activeFundingSources.reduce((acc: number, fs: any) => acc + Number(fs.monthly_budget), 0)
  const totalMonthBalance = activeFundingSources.reduce((acc: number, fs: any) => acc + Number(fs.current_month_balance), 0)
  const totalYearBalance = activeFundingSources.reduce((acc: number, fs: any) => acc + Number(fs.current_year_balance), 0)
