...
export default async function EvaluationsPage({
  ...
  const selectedName = participants?.find(
      ...
      <main className="max-w-5xl flex flex-col gap-4">
        ...
        <div className="bg-white rounded-2xl ring-1 ring-zinc-200 shadow-sm px-6 py-5">
          ...
          <p className="text-xs text-zinc-500 mb-4">
            당사자와 월을 선택하면 아래에서 바로 평가 내용을 확인할 수 있습니다.
          </p>
        ...
        {inlineData && (
          <div className="flex flex-col gap-4 animate-in fade-in duration-300">
              ...
              <div className="grid grid-cols-3 gap-4">
                <div className="flex flex-col gap-1">
                  ...
                  <p className="text-2xl font-black text-zinc-900">
                    {formatCurrency(inlineData.totalSpent)}원
                  </p>
                </div>
                <div className="flex flex-col gap-1">
                  <p className="text-[10px] font-black text-zinc-400 uppercase tracking-widest">
                ...
                <div className="flex flex-col gap-1">
                  ...
                  <p className="text-2xl font-black text-zinc-900">
                    {inlineData.planProgress.length}개
                  </p>
                </div>
              </div>
            </SectionCard>
            ...
            <SectionCard
              ...
              defaultOpen={true}
              badge={
                inlineData.planProgress.some(
                  (p: any) => !p.easy_description,
                ) && (
                  ...
                  <div className="bg-blue-50 border border-blue-100 rounded-xl p-4">
                    ...
                    <div className="flex flex-wrap gap-2">
                      {inlineData.planProgress.map((p: any) => (
                        <div
                          key={p.id}
                          className="flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-white ring-1 ring-blue-200 text-xs"
                        >
                          <span className="w-5 h-5 rounded-lg bg-zinc-900 text-white font-black text-[10px] flex items-center justify-center shrink-0">
                            {p.order_index}
                          </span>
                          {p.easy_description ? (
